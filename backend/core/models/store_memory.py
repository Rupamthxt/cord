...
def run_incremental_intelligence(points_data: List[Tuple[str, str, List[float], Dict[str, Any]]]):
    ...
    """
    try:
        from backend.core.services.correlation_engine import CrossSourceCorrelationEngine
        from backend.connectors.ingestion.event_extractor import EventExtractor
        ...
        for chunk_id, text, embedding, payload in points_data:
            ...
            if event:
                try:
                    from backend.core.services.pattern_detector import PatternDetector
            ...
            # 4. Run operational event extraction (Ollama → PostgreSQL graph)
            try:
                from backend.graph.events.pipeline import event_extraction_pipeline
                workspace_id = payload.get("workspace_id", "default_workspace")
                event_extraction_pipeline.process_chunk_sync(
                    chunk_text=text,
                    chunk_id=chunk_id,
                    workspace_id=workspace_id,
                    metadata=payload
                ...
                logger.warning(f"Event extraction skipped for chunk {chunk_id}: {ee}")
